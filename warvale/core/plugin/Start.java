...
public class Start {
	...
	if (Main.getRedTeam().getEntries().toArray().length >= 1 && Main.getBlueTeam().getEntries().toArray().length >= 1) {
		BukkitRunnable bukkitRunnable = new BukkitRunnable() {
			...
			public void run() {
				if (!(time <= 0)) {
					time--;
					Bukkit.broadcastMessage(ChatColor.DARK_RED + "CTF" + ChatColor.GRAY + " starts in " + time + " seconds.");
				} else {
					time = 10;
